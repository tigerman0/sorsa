...
class ExtCaidInfo(Poll, Converter, object):
	CAID = 0
	PID = 1
	PROV = 2
	ALL = 3
						...
						if self.type == self.SHORT:
							if source == "emu":
								...
								try:
									...
									pass
						if self.type == self.MEDIUM:
							if source == "emu":
								textvalue = "%s - %s (Prov: %s, Caid: %s)" % (source, self.systemTxtCaids.get(caid[:2]), prov, caid)
							elif server == "" and port == "": 
								textvalue = "Caid: %s  Reader: %s - %s" % (caid, reader, ecm_time.replace('msec','ms'))
							else:
								try:
									textvalue = "Caid: %s  Reader: %s - %s" % (caid, reader, ecm_time.replace('msec','ms'))
								except:
