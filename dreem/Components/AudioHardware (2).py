# Embedded file name: /usr/lib/enigma2/python/Components/AudioHardware.py
...
class AudioHardware:
        ...
        if SystemInfo['HasAC3PlusSupport']:
            ...
            if SystemInfo['SupportsAC3PlusTranscode']:
                ...
                if not choices:
                    ...
                    return
                Log.w('%s' % (choices,))
                config.av.convert_ac3plus = ConfigSelection(choices=choices, default=self.modeToKey(self._audioManager.getDefaultMode(eAudioManager.OUTPUT_HDMI, eAudioManager.FMT_AC3_PLUS)))
    ...
    def modeToString(self, index):
        return {eAudioManager.MODE_AUTO: _('controlled by HDMI'),
         eAudioManager.MODE_FORCE_AC3: _('always')}[index]

    def modeToKey(self, index):
        return {eAudioManager.MODE_AUTO: 'use_hdmi_caps',
         eAudioManager.MODE_FORCE_AC3: 'always'}[index]

    def modeToIndex(self, mode):
        return {'use_hdmi_caps': eAudioManager.MODE_AUTO,
         'always': eAudioManager.MODE_FORCE_AC3}[mode]
